...
%Tmax,峰值温度
    ...
    T05 = Ts4 + (T04 - Ts4).*exp(k(4)/v.*(314.5-246));
    T06 = Ts5 + (T05 - Ts5).*exp(k(5)/v.*(319.5-314.5));
    %分段函数
    T = (Ts1 + (T01 - Ts1).*exp(k(1)/v.*x)).*(x>=0 & x<=175) + ...
        (Ts2 + (T02 - Ts2).*exp(k(2)/v.*(x-175))).*(x>175 & x<=210.5)+...
        (Ts3 + (T03 - Ts3).*exp(k(3)/v.*(x-210.5))).*(x>210.5 & x<=246)+...
        (Ts4 + (T04 - Ts4).*exp(k(4)/v.*(x-246))).*(x>246 & x<=314.5)+...
        (Ts5 + (T05 - Ts5).*exp(k(5)/v.*(x-314.5))).*(x>314.5 & x<=319.5)+...
        (Ts6 + (T06 - Ts6).*exp(k(6)/v.*(x-319.5))).*(x>319.5 & x<=410.5);
    %求斜率
    grad(1) = -k(1)*(Ts1-T01);
    grad(2) = -k(2)*(Ts2-T02);
    grad(3) = -k(3)*(Ts3-T03);
